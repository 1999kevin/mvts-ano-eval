...
class MyDataset(object):
    def __init__(self,
        ...
        self.X = entities[0].Y.transpose((1,0))
        self.label = entities[0].labels
        ...
        # asserts
        assert all([(entity.n_features == self.n_features) for entity in
                    self.entities]), 'All entities must have same number of features!'
        assert all([(entity.n_exogenous == self.n_exogenous) for entity in
                    self.entities]), 'All entities must have same number of exogenous variables!'
